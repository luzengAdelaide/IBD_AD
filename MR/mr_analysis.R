...
################### IBD/CD/UC as exposure, AD as outcome ###################
gsmr_data = read.table("use.trait.ad.gwas" ,sep="\t", header=T)
gsmr_data = gsmr_data[-10,]

############# calculate ld
# gcta-1.94.1 --bfile wgs.rosmap --extract trait.ad.allele --update-ref-allele trait.ad.allele --recode --out gsmr_trait

# Estimate LD correlation matrix using R
snp_coeff_id = scan("gsmr_trait.xmat.gz", what="", nlines=1)
snp_coeff = read.table("gsmr_trait.xmat.gz", header=F, skip=2)
...
n_ref = 59957    # Sample size of the reference sample
...
plot(bzx[filtered_index], bzy[filtered_index], pch=20, cex=0.8, bty="n", cex.axis=1.1, cex.lab=1.2,
     ...
     xlab=expression(trait~(italic(b[zx]))),
     ylab=expression(AD~(italic(b[zy]))),
     main = bquote( atop(b[xy] ~'=-0.005' , 
                         'p-value = 6.71E-03')))
...
#########################
# Method 2: TwoSampleMR #
#########################
rm(list=ls())
library(TwoSampleMR)

ibd_file <- "FILEPATH/use.ibd.ad.gwas"
cd_file <- "FILEPATH/use.cd.ad.gwas"
uc_file <- "FILEPATH/use.uc.ad.gwas"

################### IBD as exposure, AD as outcome ################### 
ibd_exp_dat <- read_exposure_data(
  filename = ibd_file,
  sep = "\t",
  snp_col = "SNP",
  beta_col = "bzx",
...
################### UC as exposure, AD as outcome ################### 
uc_exp_dat <- read_exposure_data(
  filename = uc_file,
  sep = "\t",
  snp_col = "SNP",
  beta_col = "bzx",
