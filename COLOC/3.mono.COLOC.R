...
gwas_window_dir <-  "FILEPATH/mono_1MB/trait_1MB"

# Load eQTL for each risk snp for each cell type
eqtl_window_dir <-  "FILEPATH/mono_1MB/trait_1MB"
...
for(eqtl_window_file in eqtl_windows) {
  ...
  df$varbeta <- (df$se.y)^2
  df$varbeta2 <- (df$se.x)^2
  ...
  dir = "/FILEPATH/mono_1MB/trait_1MB"
